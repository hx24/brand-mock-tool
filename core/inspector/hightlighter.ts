import { ElementBounds, getElementBounds, getElementMockId } from './element'
...
export async function highlight(instance: HTMLElement) {
  if (!instance) return

  const bounds = getElementBounds(instance)
  if (bounds) {
    createOverlay()

    // Name
    const name = getElementMockId(instance)
...
function showOverlay(bounds: ElementBounds, children: HTMLElement[]) {
...
async function updateOverlay() {
  if (currentInstance) {
    const bounds = getElementBounds(currentInstance)
    if (bounds) {
      // 更新宽高文本
